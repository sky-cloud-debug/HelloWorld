...
function initChart(charType,timeType,startTime,endTime){
	$.ajax({
		url:"${pageContext.request.contextPath}/region/detail/soilStatisticData2",
	...
	function initCharts(json){
		...
		for(var i=0;i<json.length;i++){
			...
			if(json[i].soilHumi=="--"){humiArr.push(parseFloat(-333));}else{humiArr.push(parseFloat(json[i].soilHumi));}
			if(json[i].elect=="--"){electArr.push(parseFloat(-333));}else{electArr.push(parseFloat(json[i].elect));}
			if(json[i].ph=="--"){phArr.push(parseFloat(-333));}else{phArr.push(parseFloat(json[i].ph));}
		...
		$("#highcharts_container").highcharts({ //图表展示容器，与div的id保持一致
			...
			series:[{"name":"土壤温度",data:tempArr,tooltip: {
                valueSuffix: ' °C'
            }},{"name":"土壤湿度",data:humiArr,tooltip: {
                valueSuffix: ' %'
            }},{"name":"土壤电导率",data:electArr,tooltip: {
                valueSuffix: ' ms/cm'
            }},{"name":"土壤PH值",data:phArr,tooltip: {
                valueSuffix: ' '
            }}],//统计项的数据
			credits:{
				enabled:true,
