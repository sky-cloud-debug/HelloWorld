...
href="${pageContext.request.contextPath}/system.jsp">数据挖掘</a><i class="fa fa-angle-right"></i><span class="athere">气象数据分析</span>
				</div>
				<div class="content-view">
				<p>   
				    <span class="c_title" style="margin-left: -20px;">图形：&nbsp;&nbsp;&nbsp;</span>
					<select id="charType" style="width:100px;"onchange="changeChartType(this)"style="width:80px;">
					...
					<select id="charTime"style="width:100px;">
					     <option value="true" style="padding-left: 12px;">天</option>
					     <option value="false" style="padding-left: 12px">小时</option>
					</select>
					</br>
...
function initChart(charType,timeType,startTime,endTime){
	$.ajax({
		url:"${pageContext.request.contextPath}/region/detail/weatherStatisticData2",
		data:{plotId:getCookie("byd"),isDay:timeType,startTimeStr:startTime,endTimeStr:endTime},
		type:"post",
		dataType:"json",
	...
	function initCharts(json){
		...
		for(var i=0;i<json.length;i++){
			tempArr.push(parseFloat(json[i].airTemp));
			windSpeedArr.push(parseFloat(json[i].windSpeed));
			rainHourArr.push(parseFloat(json[i].rainHour));          
                        sunDataArr.push(parseFloat(json[i].sunData));
			humiArr.push(parseFloat(json[i].airHumi));
                        co2Arr.push(parseFloat(json[i].co2));
		...
		$("#highcharts_container").highcharts({ //图表展示容器，与div的id保持一致
			...
			series:[{"name":"空气温度",data:tempArr,tooltip: {
                valueSuffix: ' °C'
            }},{"name":"空气湿度",data:humiArr,tooltip: {
                valueSuffix: ' %'
            }},{"name":"co2浓度",data:co2Arr,tooltip: {
                valueSuffix: ' ppm'
            }},{"name":"降雨量",data:rainHourArr,tooltip: {
                valueSuffix: ' mm'
            }},

{"name":"太阳辐射",data:sunDataArr,tooltip: {
    valueSuffix: ' lux'
}},{"name":"风速",data:windSpeedArr,tooltip: {
    valueSuffix: ' m/s'
}}]	       //统计项的数据
		});
	}
